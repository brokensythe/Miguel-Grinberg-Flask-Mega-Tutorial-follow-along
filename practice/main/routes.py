...
from practice.main.forms import EditProfileForm, EmptyForm, PostForm, SearchForm
from practice.models import User, Post
from practice.translate import translate
from practice.main import bp
...
def explore():
    ...
    prev_url = url_for('main.explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('explore.html', title=_('Explore'), posts=posts.items, next_url=next_url, prev_url=prev_url, form=form)
...
def delete(id):
    ...
    if form.validate_on_submit():
        ...
        if post is None:
            flash(_('Post could not be located.'))
            return redirect(url_for('main.index'))
        db.session.delete(post)
        db.session.commit()
        flash(_('You have successfully deleted your post.'))
        return redirect(url_for('main.index'))
    else:
        return redirect(url_for('main.index'))
